    ...
    M: SmtpMailer + Clone + Send + Sync + 'static,
{
    ...
    pub(crate) async fn login(
        ...
        let user = match tx.get_user_by_username(username.clone()).await {
            Ok(user) => user,
        ...
        tx.update_user(username.clone(), now).await?;
...
mod tests {
    ...
    async fn test_login_ok_first_time() {
        ...
        let mut tx = context.tx().await;
        let session = tx.get_session(response.access_token()).await.unwrap();
    ...
    async fn test_login_ok_returning() {
        ...
        .await
        .unwrap();
        tx.update_user(username.clone(), OffsetDateTime::from_unix_timestamp(1).unwrap())
            .await
            .unwrap();
        tx.commit().await.unwrap();
    ...
    async fn test_login_not_allowed() {
        ...
        let mut tx = context.tx().await;
        tx.create_user(username.clone(), None, EmailAddress::from("some@example.com"))
            .await
            .unwrap();
        tx.commit().await.unwrap();
    ...
    async fn test_login_not_activated() {
        ...
        let user = tx
            ...
            .await
            .unwrap();
        tx.set_user_activation_code(user, Some(50)).await.unwrap();
        tx.commit().await.unwrap();
