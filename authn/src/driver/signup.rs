    ...
    M: Messenger + Clone + Send + Sync + 'static,
{
    ...
    pub(crate) async fn signup(
        ...
        self.messenger
            .send_activation_code(&self.base_url, user.username(), user.email(), activation_code)
            .await?;
...
mod tests {
    ...
    async fn test_signup_ok() {
        ...
        assert_eq!(
            ...
            context.get_latest_activation_code(user.email(), &username)
        );
    ...
    async fn test_signup_username_already_exists() {
        ...
        assert!(context.get_latest_activation_code(&email, &username).is_none());
    }
    ...
    async fn test_signup_email_already_exists() {
        ...
        assert!(context.get_latest_activation_code(&email, &Username::from("x")).is_none());
    }
